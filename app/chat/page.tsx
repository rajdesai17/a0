...
Focus only on what's needed for this specific use case. Be concise and practical.`
  ...
  const generateInstructions = async (code: string, userRequest?: string, docContext?: any): Promise<void> => {
    ...
    // Continue with rest of instructions...
    instructions += `## 🎯 Customization Guide\n\n`
    instructions += `The component uses Tailwind CSS classes for styling. Modify the classes to customize appearance.\n\n`

    instructions += `## 🐛 Troubleshooting\n\n`
    instructions += `**Common Issues:**\n`
    instructions += `- Ensure Tailwind CSS is properly configured\n`
    instructions += `- Check that all required dependencies are installed\n`
    if (hasFetch) {
      instructions += `- Verify API endpoints are accessible\n`
      instructions += `- Check environment variables are properly set\n`
    }
